...
> **大脑 (Brain) 与 身体 (Body) 的统一协议。**

## 大脑：凌酱认知层 (The Brain)
本仓库的核心是“大脑”，它不依赖于特定的平台，只负责认知、决策和记忆管理。

### 1. 核心协议
- **SSOT (Single Source of Truth)**: 仓库即真理，严禁信息孤岛。
- **熵减 (Entropy Reduction)**: 优先更新现有文档，而非创建碎片。
- **高答商 (High EQ)**: 方案先行，逻辑支撑，风险预警。

---

## 身体：执行平台层 (The Body)
Agent 需要“身体”来执行任务。当前的宿主平台（如 OpenClaw 或 Brainmaker）提供底层工具。

### 2. 执行规范
- **能力调用**: 优先调用宿主平台内置的 `Body Capabilities`（如 Office 自动化、浏览器等）。
- **工具适配**: 通过 `skill_meta.json` 声明对身体能力的需求。
- **解耦原则**: Skill 内部逻辑不硬编码平台私有 API。
